...
// 2. User Chuyên dùng tạo mẫu Prompt
...
<requirements>
1. Language: **Simplified Chinese (Tiếng Trung giản thể)** keys.
...
// 2.1 NPC Chuyên dùng tạo mẫu Prompt
...
3. **World Consistency**: The fields MUST reflect the specific logic of the provided World Setting.
   ...
   - If the world is Cyberpunk, include "Tỷ lệ cấy ghép", "Công ty trực thuộc".
...
// 3. User Tạo nhân vật/Chỉnh sửa Prompt
...
const TEXT = {
    PANEL_TITLE: `<span class="pw-title-icon"><i class="fa-solid fa-wand-magic-sparkles"></i></span>User Trình tạo nhân vật`,
    BTN_TITLE: "Mở trình tạo cài đặt",
    TOAST_SAVE_SUCCESS: (name) => `Persona "${name}" Đã lưu và ghi đè!`,
    ...
    TOAST_WI_ERROR: "TavernHelper API Chưa tải, không thể thao tác Worldbook",
    TOAST_SNAPSHOT: "Đã lưu vào lịch sử", 
    TOAST_LOAD_CURRENT: "Đã đọc nội dung hiện tại",
    TOAST_QUOTA_ERROR: "Không đủ dung lượng lưu trữ trình duyệt (Quota Exceeded),Vui lòng dọn dẹp lịch sử cũ."
};
...
function getRealSystemPrompt(selectedPreset) {
    ...
    if (window.TavernHelper && typeof window.TavernHelper.getPreset === 'function') {
        ...
        } catch (e) { console.warn("[PW] Lấy từ preset System Prompt Thất bại:", e); }
    }
...
function getPresetHintText(val) {
    if (val === 'pure') {
        return "Chế độ thuần túy giúp tránh ảnh hưởng từ phong cách preset hoặc tiếp nối cốt truyện, nhưng không có tính năng vượt rào (jailbreak). Nếu bị từ chối trả lời, vui lòng thử chuyển sang preset khác có chứa jailbreak.";
    }
    if (val === 'current') {
        return "Sẽ sử dụng preset đang kích hoạt của Tavern (Main + Jailbreak).Nếu preset hiện tại chứa lệnh tiếp nối cốt truyện mạnh, nó có thể ảnh hưởng đến kết quả tạo.";
    }
    return `Sẽ bắt buộc dùng preset chỉ định "${val}" của System Prompt để tạo.`;
}
...
async function runGeneration(data, apiConfig, isTemplateMode = false) {
    ...
    const updateDebugView = (messages) => {
        ...
        debugText += `=== Chế độ: ${isTemplateMode ? `${modeStr}Tạo mẫu` : (data.mode === 'refine' ? `${modeStr}Chỉnh sửa` : `${modeStr}Tạo nhân vật`)} ===\n`;
        debugText += `=== Chiến lược preset: ${uiStateCache.generationPreset === 'pure' ? '✨ Chế độ thuần túy (Pure Mode)' : (uiStateCache.generationPreset === 'current' ? 'Theo preset Tavern (Default)' : uiStateCache.generationPreset)} ===\n\n`;
    ...
    try {
        ...
        const doRequest = async (messages) => {
            if (apiConfig.apiSource === 'independent') {
                ...
                if (window.TavernHelper && typeof window.TavernHelper.generateRaw === 'function') {
                    ...
                    throw new Error("ST Phiên bản quá cũ hoặc chưa cài đặt TavernHelper");
                }
            }
        };
        ...
        try {
            ...
            if (prefillContent && isBadRequest) {
                ...
                toastr.info("API Quay lại 400 Lỗi (Có thể là Gemini và các mô hình khác không hỗ trợ Prefill),Đang thử lại với chế độ tương thích...");
    ...
    if (!responseContent) throw new Error("API Trả về trống (Empty Response)");
...
function loadData() {
    ...
    // Sử dụng object spread, đảm bảo generationPreset Ngay cả khi không có trong bản lưu cũ, cũng sẽ được khởi tạo bằng giá trị mặc định thành 'current'
    try {
...
function saveHistory(item) {
    ...
    if (!item.title || item.title === "Chưa đặt tên") {
        ...
        if (item.data && item.data.type === 'template') {
            item.title = mode === 'npc' ? `NPC Mẫu (${charName})` : `User Mẫu (${charName})`;
        } else {
            if (mode === 'npc') {
...
function generateSmartKeywords(name, content, staticTags = []) {
    ...
    // 1. Thử trích xuất từ nội dung "Bí danh/Biệt danh/Alias"
    const aliasMatch = content.match(/(?:Bí danh|Biệt danh|Alias)[::]\s*(.*?)(\n|$)/i);
    if (aliasMatch) {
        // Hỗ trợ phân tách bằng dấu phẩy tiếng Trung, dấu phẩy tiếng Anh, dấu phẩy liệt kê
    ...
    if (name.includes('·')) {
        // Ví dụ "Sylvie·Paula" -> Thêm "Sylvie"
        rawKeys.push(name.split('·')[0].trim());
    } else if (name.includes(' ')) {
        // Ví dụ "John Doe" -> Thêm "John" (Ngăn kích hoạt bằng một chữ cái)
        const firstName = name.split(' ')[0].trim();
        if (firstName.length > 1) rawKeys.push(firstName);
    }

    // 3. Xóa trùng, lọc từ ngắn(Độ dài<=1),Xóa giá trị rỗng
...
async function syncToWorldInfoViaHelper(userName, content) {
    ...
    // Thử từ YAML Ưu tiên đọc tên từ nội dung, nếu không có thì dùng tên được truyền vào fallback
    const nameMatch = content.match(/Tên:\s*(.*?)(\n|$)/);
    
    if (isNpc) {
        let npcName = nameMatch ? nameMatch[1].trim() : "";
        if (!npcName) {
            npcName = prompt("Không thể tự động nhận dạng NPC Tên, vui lòng nhập:", "Người qua đường A");
            if (!npcName) return; 
        }
        entryTitle = `NPC:${npcName}`;
        entryKeys = generateSmartKeywords(npcName, content, ["NPC"]);
    } else {
        // User Ưu tiên dùng YAML tên trong (có thể người dùng đã đặt tên đầy đủ trong cài đặt), dự phòng dùng tên người dùng Tavern
...
async function openCreatorPopup() {
    ...
    let presetOptionsHtml = `
        <option value="current" ${uiStateCache.generationPreset === 'current' ? 'selected' : ''}>Theo preset Tavern (Default)</option>
...
<div class="pw-wrapper">
    ...
    <div id="pw-view-editor" class="pw-view active">
        <div class="pw-scroll-area">
            ...
            <div class="pw-info-display mode-switcher">
                <div class="pw-mode-toggle-group">
                    <div class="pw-mode-item ${!isNpc ? 'active' : ''}" data-mode="user" title="User Chế độ">
                        <i class="fa-solid fa-user"></i> ${currentName}
                    </div>
                    <div class="pw-mode-item ${isNpc ? 'active' : ''}" data-mode="npc" title="NPC Chế độ">
                        <i class="fa-solid fa-user-secret"></i> NPC
                    </div>
                </div>
                <div class="pw-load-btn" id="pw-btn-load-current" style="${isNpc ? 'visibility:hidden;' : ''}">Tải nhân vật hiện tại</div>
            </div>
            ...
            <div>
                <div class="pw-tags-header">
                    ...
                    <div class="pw-tags-actions">
                        <span class="pw-tags-edit-toggle" id="pw-load-main-template" style="${isNpc ? '' : 'display:none;'} margin-right:10px;">Sử dụngUser Mẫu</span>
                ...
                <div class="pw-template-editor-area" id="pw-template-editor">
                    ...
                    <div class="pw-template-footer">
                        <button class="pw-mini-btn" id="pw-gen-template-smart" title="Tạo mẫu tùy chỉnh dựa trên thế giới và cài đặt hiện tại">Tạo mẫu</button>
            ...
            <button id="pw-btn-gen" class="pw-btn gen">${isNpc ? 'Tạo NPC Cài đặt' : 'Tạo User Cài đặt'}</button>
            ...
            <div id="pw-result-area" style="display:${activeData.hasResult ? 'block' : 'none'}; margin-top:15px;">
                ...
                <div class="pw-refine-toolbar">
                    <textarea id="pw-refine-input" class="pw-refine-input" placeholder="Nhập ý kiến, hoặc chọn văn bản ở trên rồi nhấp vào cửa sổ nổi để sửa nhanh..."></textarea>
                    <div class="pw-refine-btn-vertical" id="pw-btn-refine" title="Thực hiện trau chuốt">
                        <span class="pw-refine-btn-text">Chỉnh sửa</span>
        ...
        <div class="pw-footer">
            <div class="pw-footer-group">
                <div class="pw-compact-btn danger" id="pw-clear" title="Xóa sạch"><i class="fa-solid fa-eraser"></i></div>
                <div class="pw-compact-btn" id="pw-copy-persona" title="Sao chép nội dung"><i class="fa-solid fa-copy"></i></div>
                <div class="pw-compact-btn" id="pw-snapshot" title="Lưu lịch sử"><i class="fa-solid fa-save"></i></div>
            </div>
    ...
    <div id="pw-diff-overlay" class="pw-diff-container" style="display:none;">
        <div class="pw-diff-tabs-bar">
            ...
            <div class="pw-diff-tab" data-view="raw">
                <div>Bản gốc mới</div><div class="pw-tab-sub">Xem/Sửa</div>
            </div>
            <div class="pw-diff-tab" data-view="old-raw">
                <div>Bản gốc cũ</div><div class="pw-tab-sub">Xem/Sửa</div>
            </div>
        </div>
        ...
        <div class="pw-diff-actions">
            <button class="pw-btn danger" id="pw-diff-cancel">Hủy thay đổi</button>
    ...
    <div id="pw-view-context" class="pw-view">
        <div class="pw-scroll-area">
            ...
            <div class="pw-card-section">
                <div class="pw-row">
                    <label class="pw-section-label">Cài đặt sẵn để tạo (System Prompt)</label>
            ...
            <div class="pw-card-section">
                ...
                <div id="pw-greetings-toggle-bar" class="pw-preview-toggle-bar" style="display:none;">
                    <i class="fa-solid fa-angle-up"></i> Thu gọn bản xem trước
                </div>
    ...
    <div id="pw-view-api" class="pw-view">
        <div class="pw-scroll-area">
            <div class="pw-card-section">
                <div class="pw-row"><label>API Nguồn</label><select id="pw-api-source" class="pw-input" style="flex:1;"><option value="main" ${config.apiSource === 'main' ? 'selected' : ''}>Chính API</option><option value="independent" ${config.apiSource === 'independent' ? 'selected' : ''}>Độc lập API</option></select></div>
        ...
        <div class="pw-scroll-area">
            ...
            <div class="pw-card-section">
                <div class="pw-row">
                    ...
                    <div style="flex:1; display:flex; gap:5px;">
                        ...
                        <button class="pw-btn danger" id="pw-btn-delete-theme" title="Xóa chủ đề hiện tại" style="padding:6px 10px; display:none;"><i class="fa-solid fa-trash"></i></button>
                        <input type="file" id="pw-theme-import" accept=".css" style="display:none;">
                        <button class="pw-btn primary" id="pw-btn-import-theme" title="Nhập cục bộ .css Tệp" style="padding:6px 10px;"><i class="fa-solid fa-file-import"></i></button>
                        
                        <button class="pw-btn primary" id="pw-btn-download-template" title="Tải mẫu chủ đề" style="padding:6px 10px;"><i class="fa-solid fa-download"></i></button>
                    </div>
                </div>
            </div>

            <!-- 2. Prompt Khu vực sửa -->
            <div class="pw-card-section">
                <div class="pw-context-header" id="pw-prompt-header">
                    <span><i class="fa-solid fa-terminal"></i> Prompt Xem và sửa (User Prompt)</span>
                ...
                <div id="pw-prompt-container" style="display:none; padding-top:10px;">
                    <div class="pw-row" style="margin-bottom:8px;">
                        ...
                        <select id="pw-prompt-type" class="pw-input" style="flex:1;">
                            <option value="personaGen">User Tạo nhân vật/Lệnh trau chuốt</option>
                            <option value="npcGen">NPC Tạo nhân vật/Lệnh trau chuốt</option>
                            <option value="templateGen">User Lệnh tạo mẫu</option>
                            <option value="npcTemplateGen">NPC Lệnh tạo mẫu</option>
                        </select>
                    </div>
                    <div class="pw-var-btns">
                        ...
                        <div class="pw-var-btn" data-ins="{{charInfo}}"><span>Cài đặt nhân vật</span><span class="code">{{charInfo}}</span></div>
            ...
            <div id="pw-debug-wrapper" class="pw-card-section" style="display:none; margin-top: 10px; border-top: 1px solid var(--SmartThemeBorderColor); padding-top: 10px;">
                <div style="margin-bottom: 5px;">
                    <label style="color: var(--SmartThemeQuoteColor); font-weight:bold;"><i class="fa-solid fa-bug"></i> Xem trước nội dung gửi theo thời gian thực (Debug)</label>
                </div>
                <div style="font-size: 0.8em; opacity: 0.7; margin-bottom: 5px;">Nhấp"Tạo cài đặt"sau đó, bên dưới sẽ hiển thị nội dung thực tế gửi cho AI đầy đủ.</div>
        ...
        <div class="pw-scroll-area">
            ...
            <div class="pw-history-filters" style="display:flex; gap:5px; margin-bottom:8px;">
                <select id="pw-hist-filter-type" class="pw-input" style="flex:1;">
                    ...
                    <option value="user_persona">User Nhân vật</option>
                    <option value="npc_persona">NPC Nhân vật</option>
                    <option value="user_template">User Mẫu</option>
                    <option value="npc_template">NPC Mẫu</option>
                </select>
            ...
            <div class="pw-search-box">
                ...
                <input type="text" id="pw-history-search" class="pw-input pw-search-input" placeholder="Lịch sử tìm kiếm...">
...
`;
    ...
    updatePromise.then(updateInfo => {
        ...
        if (hasNewVersion) {
            ...
            $container.html(`<div style="margin-top:10px; opacity:0.6; font-size:0.9em;"><i class="fa-solid fa-check"></i> Hiện đã là phiên bản mới nhất</div>`);
        }
    });
    ...
    loadAvailableWorldBooks().then(() => {
        ...
        $('#pw-wi-select').html(`<option value="">-- Thêm tham chiếu/Worldbook mục tiêu --</option>${options}`);
    });
...
function bindEvents() {
    ...
    $(document).on('click.pw', '.pw-mode-item', function() {
        ...
        // 4. Update UI Buttons
        if (mode === 'npc') {
            $('#pw-btn-gen').text("Tạo NPC Cài đặt");
            $('#pw-btn-apply').hide();
            $('#pw-btn-load-current').css('visibility', 'hidden'); 
            $('#pw-load-main-template').show(); 
            toastr.info("Đã chuyển sang NPC Chế độ");
        } else {
            $('#pw-btn-gen').text("Tạo User Cài đặt");
            $('#pw-btn-apply').show();
            $('#pw-btn-load-current').css('visibility', 'visible');
            $('#pw-load-main-template').hide();
            toastr.info("Đã chuyển sang User Chế độ");
        }
    });
    ...
    $(document).on('click.pw', '#pw-btn-update', function() {
        ...
        window.TavernHelper.updateExtension(extensionName).then(res => {
            if (res.ok) {
                toastr.success("Cập nhật thành công! Đang làm mới trang...");
                setTimeout(() => window.location.reload(), 1500);
            } else {
                toastr.error("Cập nhật thất bại, vui lòng kiểm tra bảng điều khiển.");
            }
        });
    });
    ...
    $(document).on('change.pw', '#pw-theme-import', function(e) {
        ...
        reader.onload = function(e) {
            ...
            toastr.success(`Đã nhập chủ đề: ${themeName}`);
        };
    ...
    $(document).on('click.pw', '#pw-btn-delete-theme', function() {
        ...
        if (confirm(`Bạn có chắc muốn xóa chủ đề "${current}"  không?`)) {
    ...
    $(document).on('click.pw', '#pw-btn-download-template', async function() {
        ...
        if (!cssContent) return toastr.error("Không thể lấy nội dung chủ đề");
    ...
    $(document).on('change.pw', '#pw-greetings-select', function() {
        ...
        if (idx === "") {
            ...
            $toggleBtn.show().html('<i class="fa-solid fa-angle-up"></i> Mở rộng xem trước');
        }
    });
    ...
    $(document).on('click.pw', '#pw-greetings-toggle-bar', function() {
        ...
        if ($preview.is(':visible')) {
            ...
            $(this).html('<i class="fa-solid fa-angle-up"></i> Thu gọn bản xem trước');
        }
    });
    ...
    $(document).on('click.pw', '#pw-copy-persona', function() {
        ...
        toastr.success("Đã sao chép thiết lập nhân vật");
    });
    ...
    $(document).on('click.pw', '#pw-load-main-template', function() {
        if(confirm("Bạn có chắc muốn sử dụng  User mẫu chính mặc định không? Thao tác này sẽ ghi đè nội dung trình chỉnh sửa hiện tại.")) {
            ...
            toastr.success("Đã tải User Mẫu chính");
        }
    });
    ...
    $(document).on('click.pw', '#pw-reset-template-small', function() {
        ...
        if(confirm(`Bạn có chắc muốn khôi phục về  ${targetName} mẫu mặc định không?`)) {
    ...
    $(document).on('click.pw', '#pw-gen-template-smart', async function() {
        ...
        try {
            ...
            if (!hasCharInfo && !hasWi) {
                const wantGeneric = confirm("Hiện không phát hiện thông tin thẻ nhân vật hoặc Worldbook liên kết.\n\n Bạn có muốn tạo mẫu chung không?");
                ...
                const useDefault = confirm("Vui lòng chọn nguồn mẫu:\n\n Nhấp [OK] để dùng mẫu mặc định tích hợp (Khuyên dùng)\n Nhấp [Hủy] để tạo mẫu chung hoàn toàn mới");
            ...
            if (generatedTemplate) {
                ...
                toastr.success("Tạo mẫu thành công! Vui lòng nhấp"Lưu mẫu"Xác nhận sửa đổi.");
            }
        } catch (e) {
            console.error(e);
    ...
    $(document).on('mousedown.pw', '#pw-float-quote-btn', function (e) {
        ...
        if (selectedText) {
            ...
            if ($input && $input.length) {
                const cur = $input.val();
                const newText = `Đối với  "${selectedText}"  ý kiến sửa đổi là:`;
    ...
    $(document).on('click.pw', '#pw-btn-refine', async function (e) {
        ...
        if (!refineReq) {
            toastr.warning("Vui lòng nhập ý kiến trau chuốt");
            isProcessing = false;
            return;
        }
        ...
        try {
            ...
            allKeys.forEach(key => {
                ...
                if (!isChanged) {
                    ...
                    <div class="pw-diff-card new selected single-view" data-val="${encodeURIComponent(valNew)}">
                        <div class="pw-diff-label">Không thay đổi</div>
            ...
            $('.pw-diff-tab[data-view="raw"] div:first-child').text('Bản gốc mới');
            $('.pw-diff-tab[data-view="raw"] .pw-tab-sub').text('Xem/Sửa');
            $('.pw-diff-tab[data-view="old-raw"] div:first-child').text('Bản gốc cũ');
            $('.pw-diff-tab[data-view="old-raw"] .pw-tab-sub').text('Xem/Sửa');

            if (changeCount === 0 && !responseText) {
                toastr.warning("Nội dung trả về trống, vui lòng chuyển sang"Chỉnh sửa trực tiếp"Xem");
            ...
            console.error(e);
            toastr.error("Trau chuốt thất bại: " + e.message); 
        } finally { 
    ...
    $(document).on('click.pw', '#pw-btn-gen', async function (e) {
        ...
        try {
            ...
            const isNpc = uiStateCache.generationMode === 'npc';
            $btn.prop('disabled', false).html(isNpc ? 'Tạo NPC Cài đặt' : 'Tạo User Cài đặt'); 
            isProcessing = false;
        }
    });
    ...
    $(document).on('click.pw', '#pw-btn-load-current', function() {
        ...
        if (content) {
            if ($('#pw-result-text').val() && !confirm("Hộp kết quả hiện tại đã có nội dung, bạn có chắc muốn ghi đè không?")) return;
            ...
            toastr.warning("Không phát hiện mô tả thiết lập nhân vật hợp lệ");
        }
    });
    ...
    $(document).on('click.pw', '#pw-clear', function () {
        if (confirm("Chắc chắn làm trống?")) {
    ...
    $(document).on('click.pw', '#pw-api-save', () => {
        ...
        saveData();
        toastr.success("Prompt Đã lưu");
    });

    $(document).on('click.pw', '#pw-reset-prompt', () => {
        if (!confirm("Chắc chắn khôi phục mặc định Prompt?")) return;
        const type = $('#pw-prompt-type').val();
        if (type === 'templateGen') {
...
// Tải động bên ngoài CSS Tệp (Dùng cho style.css)
...
// Áp dụng tùy chỉnh CSS Nội dung (Chủ đề dùng để nhập)
...
const renderHistoryList = () => {
    ...
    historyCache.forEach(item => {
        ...
        // NPC: "NPC:Name @ Char"
        // User: "User & Char" or "User Mẫu (Char)"
        let charName = "";
        if (title.includes(' @ ')) {
    ...
    if (paginated.length === 0) { $list.html('<div style="text-align:center; opacity:0.6; padding:20px;">Tạm thời không có bản ghi</div>'); return; }
            ...
            <div class="pw-hist-main">
                <div class="pw-hist-header">
                    ...
                    <div style="display:flex; gap:5px; flex-shrink:0;">
                        <i class="fa-solid fa-pen pw-hist-action-btn edit" title="Chỉnh sửa tiêu đề"></i>
...
const renderWiBooks = async () => {
    ...
    if (allBooks.length === 0) { 
        container.html('<div style="opacity:0.6; padding:10px; text-align:center;">Nhân vật này chưa liên kết Worldbook, vui lòng thêm thủ công ở tab "Worldbook" hoặc liên kết trên giao diện chính của Tavern.</div>'); 
        return; 
    }
    ...
    for (const book of allBooks) {
        ...
        <div class="pw-wi-book">
            <div class="pw-wi-header" style="display:flex; align-items:center;">
                <input type="checkbox" class="pw-wi-header-checkbox pw-wi-select-all" title="Chọn tất cả/Bỏ chọn tất cả (Chỉ chọn các mục hiện hiển thị)">
                ...
                if (!$list.data('loaded')) {
                    ...
                    if (entries.length === 0) {
                        ...
                        <div class="pw-wi-depth-tools">
                            ...
                            <div class="pw-wi-filter-row">
                                <select id="p-select" class="pw-pos-select">
                                    ...
                                    <option value="before_author_note">AN Trước</option>
                                    <option value="after_author_note">AN Sau</option>
                                ...
                                <input type="number" class="pw-depth-input" id="d-min" placeholder="0" title="Sâu tối thiểu">
                                <span>-</span>
                                <input type="number" class="pw-depth-input" id="d-max" placeholder="Max" title="Sâu tối đa">
                            </div>
                            <div class="pw-wi-filter-row">
                                <button class="pw-depth-btn" id="d-filter-toggle" title="Bật/Hủy lọc">Lọc</button>
                                <button class="pw-depth-btn" id="d-clear-search">Xóa nội dung</button>
                                <button class="pw-depth-btn" id="d-reset" title="Khôi phục trạng thái gốc">Reset trạng thái</button>
                            </div>
                        </div>`);
                        ...
                        $tools.find('#d-reset').on('click', function() {
                             ...
                             toastr.info("Đã khôi phục trạng thái gốc");
                        });
